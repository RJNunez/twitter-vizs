...
#### -- DON'T RUN -- ####
# library(data.table)
# url <- "https://earthquake.usgs.gov/fdsnws/event/1/query.csv?starttime=1500-01-01%2000:00:00&endtime=2020-01-23%2023:59:59&maxlatitude=18.684&minlatitude=17.566&maxlongitude=-65.094&minlongitude=-67.972&minmagnitude=1&orderby=time"
# dat <- fread(url)
# dat <- dat %>%
#   as_tibble() %>%
...
# -- Loading data
dat_pr   <- read_csv("temblores-pr/data/seismic-activity-pr.csv") %>%
...
for(y in sort(unique(year(dat_pr$date))))
{
  ...
  ggsave(filename = paste0("temblores-pr/figs/mag-depth-temblores-",y,".pdf"),
         plot = p,
         height = 4, width = 6,
         units = "in",
         dpi = 300)
...
municipios <- st_read("temblores-pr/data/pr-shapefiles/g03_legales_municipios_edicion_octubre2015.shp") %>%
    st_transform(4326)
...
for(y in sort(unique(year(dat_pr$date))))
{
  ...
  ggsave(filename = paste0("temblores-pr/figs/temblores-",y,".pdf"),
         plot = p,
         height = 4, width = 6,
         units = "in",
         dpi = 300)
